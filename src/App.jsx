...
// ─── CONFIG ────────────────────────────────────────────────────────────────
// Fill these in after setting up Supabase (see handleiding)
const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL;
const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY;
const ADMIN_PASSWORD = import.meta.env.VITE_ADMIN_PASSWORD || "dinnertje";

// ─── SUPABASE HELPERS ──────────────────────────────────────────────────────
const sb = {
  async getResponses() {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/responses?select=*&order=submitted_at.asc`, {
      headers: {
  ...
  async insertResponse(name, dates) {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/responses`, {
      ...
      body: JSON.stringify({ name, dates, submitted_at: new Date().toISOString() }),
    });
    if (!res.ok) throw new Error(await res.text());
  },
  async deleteAll() {
...
function AdminView({ onExit }) {
  ...
  const handleClear = async () => {
    ...
    try {
      await sb.deleteAll();
      setResponses([]);
    } catch(e) { alert("Fout bij wissen: " + e.message); }
  };
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#fff", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', sans-serif" }}>
      <div style={{ borderBottom: "1px solid #f0f0f0", padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {!loading && <span style={{ fontSize: 13, color: "#bbb" }}>{responses.length} respons{responses.length !== 1 ? "en" : ""}</span>}
        </div>
      ...
      {loading ? (
        <div style={{ padding: 64, textAlign: "center", color: "#bbb" }}>Laden…</div>
      ) : error ? (
        <div style={{ padding: 48, textAlign: "center" }}>
          <div style={{ fontSize: 28, marginBottom: 12 }}>⚠️</div>
          <div style={{ color: "#c00", fontSize: 14, marginBottom: 8 }}>Kan geen verbinding maken met de database.</div>
          <div style={{ color: "#aaa", fontSize: 12, maxWidth: 400, margin: "0 auto" }}>{error}</div>
        </div>
      ) : responses.length === 0 ? (
              ...
              {responses.map((r, i) => (
                <div key={i} style={{ marginBottom: 24, paddingBottom: 24, borderBottom: i < responses.length - 1 ? "1px solid #f0f0f0" : "none" }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 10 }}>
                    ...
                    <span style={{ fontSize: 12, color: "#ccc" }}>{new Date(r.submitted_at).toLocaleDateString("nl-NL", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</span>
...
export default function App() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null);
    try {
      await sb.insertResponse(name, dateList);
      setScreen("thanks");
    } catch(e) {
    ...
    <div style={S.root}>
      <div style={{ ...S.card, maxWidth: 480 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          ...
          <button style={{ ...S.button, padding: "10px 18px", fontSize: 13, opacity: totalSelected > 0 && !submitting ? 1 : 0.35, cursor: totalSelected > 0 && !submitting ? "pointer" : "default" }}
            disabled={totalSelected === 0 || submitting} onClick={handleSubmit}>
            {submitting ? "…" : "Submit"}
          </button>
        </div>
        ...
        {submitError && <p style={{ color: "#c00", fontSize: 13, marginBottom: 16 }}>{submitError}</p>}
        ...
        <button style={{ ...S.button, width: "100%", opacity: totalSelected > 0 && !submitting ? 1 : 0.35, cursor: totalSelected > 0 && !submitting ? "pointer" : "default" }}
          disabled={totalSelected === 0 || submitting} onClick={handleSubmit}>
          {submitting ? "Opslaan…" : "Submit"}
        </button>
      </div>
    </div>
  );
